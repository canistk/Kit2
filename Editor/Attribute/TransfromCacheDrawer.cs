...
namespace Kit2
{
	...
	public class TransformCacheDrawer : PropertyDrawer
	{
		...
		struct VariablesGroup
		{
			...
			{
				localPositionProp = _property.FindPropertyRelative(nameof(TransformCache.m_LocalPosition));
				localRotationProp = _property.FindPropertyRelative(nameof(TransformCache.m_LocalRotation));
				localScaleProp = _property.FindPropertyRelative(nameof(TransformCache.m_LocalScale));
				anchorMinProp = _property.FindPropertyRelative(nameof(TransformCache.m_AnchorMin));
				anchorMaxProp = _property.FindPropertyRelative(nameof(TransformCache.m_AnchorMax));
				sizeDeltaProp = _property.FindPropertyRelative(nameof(TransformCache.m_SizeDelta));
				pivotProp = _property.FindPropertyRelative(nameof(TransformCache.m_Pivot));
				parentProp = _property.FindPropertyRelative(nameof(TransformCache.m_Parent));
				tranRefProp = _property.FindPropertyRelative(nameof(TransformCache.m_TransformRef));
			}
		}
		...
		public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
		{
			...
			if (GUI.Button(left[0], l_recordButton))
			{
				...
				for (int i = 0; i < property.serializedObject.targetObjects.Length; i++)
				{
					...
					_data.parentProp.objectReferenceValue = _component.transform.parent;
			...
			if (GUI.Button(cols[0], l_AssignButton))
			{
				...
				for (int i = 0; i < property.serializedObject.targetObjects.Length; i++)
				{
					...
					cache.AssignTo(_component.transform, cache.m_TransformRef);
					_property.serializedObject.ApplyModifiedProperties();
				}
			}
			...
			bool isMultipleSelection = property.serializedObject.targetObjects.Length > 1;
			if (isMultipleSelection)
			{
				EditorGUI.HelpBox(cols[1].Adjust(x: 15f, width: -15f), "Not support multiple modify for detail.", MessageType.Info);
				return;
			}
			property.isExpanded = EditorGUI.Foldout(cols[1].Adjust(x: 15f).Clone(width: 50f), property.isExpanded, l_Detail, true);
			eTransformCacheRef tranRef = (eTransformCacheRef)data.tranRefProp.enumValueIndex;
			...
			if (property.isExpanded)
			{
				if (component.transform is RectTransform)
				{
					...
					DrawRectButton(line, data.tranRefProp,
						eTransformCacheRef.AnchorMin, data.anchorMinProp,
						() => { data.anchorMinProp.vector2Value = rectTran.anchorMin; });

					line = line.GetRectBottom(height: singleLine);
					DrawRectButton(line, data.tranRefProp,
						eTransformCacheRef.AnchorMax, data.anchorMaxProp,
						() => { data.anchorMaxProp.vector2Value = rectTran.anchorMax; });

					line = line.GetRectBottom(height: singleLine);
					DrawRectButton(line, data.tranRefProp,
						eTransformCacheRef.SizeDelta, data.sizeDeltaProp,
						() => { data.sizeDeltaProp.vector2Value = rectTran.sizeDelta; });


					line = line.GetRectBottom(height: singleLine);
					DrawRectButton(line, data.tranRefProp,
						eTransformCacheRef.Pivot, data.pivotProp,
				...
				line = line.GetRectBottom(height: singleLine);
				DrawRectButton(line, data.tranRefProp,
					eTransformCacheRef.LocalPosition, data.localPositionProp,
					() =>
					{
						if (component.transform is RectTransform)
				...
				line = line.GetRectBottom(height: singleLine);
				DrawRectButton(line, data.tranRefProp,
					eTransformCacheRef.LocalRotation, data.localRotationProp,
				...
				line = line.GetRectBottom(height: singleLine);
				DrawRectButton(line, data.tranRefProp,
					eTransformCacheRef.LocalScale, data.localScaleProp,
					() => { data.localScaleProp.vector3Value = component.transform.localScale; });

				// parent
				line = line.GetRectBottom(height: singleLine);
				DrawRectButton(line, data.tranRefProp,
					eTransformCacheRef.Parent, data.parentProp,
					() => { data.parentProp.objectReferenceValue = component.transform.parent; });
			}
		...
		private void DrawRectButton(Rect line,
			...
			eTransformCacheRef param,
			SerializedProperty relatedProp,
			System.Action btnCallback,
		...
		delegate void ToggleFunc(Rect line, SerializedProperty tranRefProp, eTransformCacheRef param, SerializedProperty relatedProp);
		...
		private void DrawDefaultParam(
			...
			SerializedProperty relatedProp)
		{
			eTransformCacheRef tranRef = (eTransformCacheRef)tranRefProp.intValue;
		...
		public static float GetPropertyHeight(SerializedProperty property)
		{
			...
			else
			{
				Component component = (Component)property.serializedObject.targetObject;
				if (component.transform is RectTransform)
					return 10f * singleLine;
				else
					return 6f * singleLine;
			}
		}
		...
		private TransformCache CloneTransformCache(VariablesGroup data)
		{
			...
			{
				m_TransformRef = (eTransformCacheRef)data.tranRefProp.intValue,
				m_Parent = data.parentProp.objectReferenceValue as Transform,
				m_LocalPosition = data.localPositionProp.vector3Value,
				m_LocalRotation = data.localRotationProp.quaternionValue,
				m_LocalScale = data.localScaleProp.vector3Value,
				m_AnchorMin = data.anchorMinProp.vector2Value,
				m_AnchorMax = data.anchorMaxProp.vector2Value,
				m_SizeDelta = data.sizeDeltaProp.vector2Value,
				m_Pivot = data.pivotProp.vector2Value,
			};
			return rst;
		}
	}
